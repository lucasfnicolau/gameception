...
public class Scene0: SKScene, SKPhysicsContactDelegate {
    ...
    public override func sceneDidLoad() {
        ...
        gameTimer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(spawn), userInfo: nil, repeats: true)
    ...
    public func yelDidCollideWithPortal(playerNode: SKSpriteNode, objectNode: SKSpriteNode) {
        ...
        if objectNode.name == "Portal" {
            objectNode.removeFromParent()
            if let scene1 = Scene1(fileNamed: "_Scene1") {
                let transition = SKTransition.flipVertical(withDuration: 1.0)
                scene1.scaleMode = .aspectFill
                scene?.view?.presentScene(scene1, transition: transition)
            }
        }
