...
public class Scene1: SKScene, SKPhysicsContactDelegate {
    ...
    public override func sceneDidLoad() {
        ...
        gameTimer = Timer.scheduledTimer(timeInterval: 0.8, target: self, selector: #selector(spawn), userInfo: nil, repeats: true)
    ...
    public func yelDidCollideWithPortal(playerNode: SKSpriteNode, objectNode: SKSpriteNode) {
        ...
        if objectNode.name == "Portal" {
            objectNode.removeFromParent()
            if let scene2 = Scene2(fileNamed: "_Scene2") {
                let transition = SKTransition.flipVertical(withDuration: 1.0)
                scene2.scaleMode = .aspectFill
                scene?.view?.presentScene(scene2, transition: transition)
            }
        }
