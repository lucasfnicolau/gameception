...
public class Scene2: SKScene, SKPhysicsContactDelegate {
    ...
    public override func sceneDidLoad() {
        ...
        gameTimer = Timer.scheduledTimer(timeInterval: 0.7, target: self, selector: #selector(spawn), userInfo: nil, repeats: true)
    ...
    public func yelDidCollideWithPortal(playerNode: SKSpriteNode, objectNode: SKSpriteNode) {
        ...
        if objectNode.name == "Portal" {
            objectNode.removeFromParent()
            if let scene3 = Scene3(fileNamed: "_Scene3") {
                let transition = SKTransition.flipVertical(withDuration: 1.0)
                scene3.scaleMode = .aspectFill
                scene?.view?.presentScene(scene3, transition: transition)
            }
        }
